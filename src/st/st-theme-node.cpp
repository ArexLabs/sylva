...
/* Custom deleters for GObject/GLib types */
struct GObjectDeleter {
    void operator()(gpointer p) const { if (p) g_object_unref(p); }
};
struct GFreeDeleter {
    void operator()(gpointer p) const { if (p) g_free(p); }
};
struct GStrvDeleter {
    void operator()(GStrv p) const { if (p) g_strfreev(p); }
};
...
st_theme_node_finalize (GObject *object)
{
  ...
  node->element_id.reset();
  node->element_classes.reset();
  node->pseudo_classes.reset();
  node->inline_style.reset();

  maybe_free_properties (node);

  node->font_desc.reset();
  ...
  node->background_image.reset();
  node->background_bumpmap.reset();

  _st_theme_node_free_drawing_state (node);

  /* Call destructors for C++ fields */
  std::destroy_at(&node->font_desc);
  std::destroy_at(&node->background_image);
  std::destroy_at(&node->background_bumpmap);
  std::destroy_at(&node->element_id);
  std::destroy_at(&node->element_classes);
  std::destroy_at(&node->pseudo_classes);
  std::destroy_at(&node->inline_style);
                   ...
                   gboolean           important)
{
  ...
  node = ST_THEME_NODE (g_object_new (ST_TYPE_THEME_NODE, NULL));

  /* Placement new for unique_ptrs inside the zero'd GObject struct */
  new (&node->font_desc) std::unique_ptr<PangoFontDescription, PangoFontDeleter>();
  new (&node->background_image) std::unique_ptr<char, GFreeDeleter>();
  new (&node->background_bumpmap) std::unique_ptr<char, GFreeDeleter>();
  new (&node->element_id) std::unique_ptr<char, GFreeDeleter>();
  new (&node->element_classes) std::unique_ptr<char*, GStrvDeleter>();
  new (&node->pseudo_classes) std::unique_ptr<char*, GStrvDeleter>();
  new (&node->inline_style) std::unique_ptr<char, GFreeDeleter>();

  node->context = ST_THEME_CONTEXT (g_object_ref (context));
  if (parent_node != NULL)
    node->parent_node = ST_THEME_NODE (g_object_ref (parent_node));
  else
    node->parent_node = NULL;
  ...
  node->element_id.reset(g_strdup (element_id));
  node->element_classes.reset(split_on_whitespace (element_class));
  node->pseudo_classes.reset(split_on_whitespace (pseudo_class));
  node->inline_style.reset(g_strdup (inline_style));
...
st_theme_node_get_element_classes (StThemeNode *node)
{
  ...
  return node->element_classes.get();
}
...
st_theme_node_get_pseudo_classes (StThemeNode *node)
{
  ...
  return node->pseudo_classes.get();
}
...
st_theme_node_equal (StThemeNode *node_a, StThemeNode *node_b)
{
  ...
  if (node_a->parent_node != node_b->parent_node ||
      ...
      g_strcmp0 (node_a->element_id.get(), node_b->element_id.get()) ||
      g_strcmp0 (node_a->inline_style.get(), node_b->inline_style.get()))
    return FALSE;
  ...
  if (node_a->element_classes != NULL)
    {
      ...
      for (i = 0; ; i++)
        {
          if (g_strcmp0 (node_a->element_classes.get()[i],
                         node_b->element_classes.get()[i]))
            return FALSE;

          if (node_a->element_classes.get()[i] == NULL)
            break;
        }
    }
  ...
  if (node_a->pseudo_classes != NULL)
    {
      ...
      for (i = 0; ; i++)
        {
          if (g_strcmp0 (node_a->pseudo_classes.get()[i],
                         node_b->pseudo_classes.get()[i]))
            return FALSE;

          if (node_a->pseudo_classes.get()[i] == NULL)
            break;
        }
    }
...
st_theme_node_hash (StThemeNode *node)
{
  ...
  if (node->element_id != NULL)
    hash = hash * 33 + g_str_hash (node->element_id.get());

  if (node->inline_style != NULL)
    hash = hash * 33 + g_str_hash (node->inline_style.get());

  if (node->element_classes != NULL)
    {
      gchar **it;

      for (it = node->element_classes.get(); *it != NULL; it++)
  ...
  if (node->pseudo_classes != NULL)
    {
      ...
      for (it = node->pseudo_classes.get(); *it != NULL; it++)
        hash = hash * 33 + g_str_hash (*it) + 1;
    }
