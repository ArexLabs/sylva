...
struct GHashTableDeleter {
    void operator()(GHashTable* p) const { if (p) g_hash_table_unref(p); }
};
struct PangoFontDeleter {
    void operator()(PangoFontDescription* p) const { if (p) pango_font_description_free(p); }
};

struct _StThemeContext {
  GObject parent;

  std::unique_ptr<PangoFontDescription, PangoFontDeleter> font;
  StThemeNode *root_node;
  StTheme *theme;

  std::unique_ptr<GHashTable, GHashTableDeleter> nodes;
...
st_theme_context_finalize (GObject *object)
{
  ...
  // RAII handles hash table, font via unique_ptr
...
st_theme_context_init (StThemeContext *context)
{
  ...
  context->nodes.reset(g_hash_table_new_full ((GHashFunc) st_theme_node_hash,
                               ...
                               GParamSpec   *pspec)
{
  ...
  switch (prop_id)
    ...
    case PROP_SCALE_FACTOR:
      {
        double scale_factor = g_value_get_double (value);
                               ...
                               GParamSpec *pspec)
{
  ...
  switch (prop_id)
    {
    case PROP_SCALE_FACTOR:
      g_value_set_double (value, context->scale_factor);
      break;
    default:
      G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
      break;
                           ...
                           const PangoFontDescription *font)
{
  ...
  context->font.reset(pango_font_description_copy (font));
  st_theme_context_changed (context);
}
...
{
  g_return_val_if_fail (stage_context != NULL, 1.0);
  // g_printerr ("theme scale: %f\n", stage_context->scale_factor);
